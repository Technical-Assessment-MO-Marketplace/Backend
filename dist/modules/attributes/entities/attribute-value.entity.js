"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AttributeValue = void 0;
const typeorm_1 = require("typeorm");
let AttributeValue = class AttributeValue {
    id;
    attribute_id;
    value;
    attribute;
    variantAttributes;
};
exports.AttributeValue = AttributeValue;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], AttributeValue.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'int' }),
    __metadata("design:type", Number)
], AttributeValue.prototype, "attribute_id", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'varchar', length: 255 }),
    __metadata("design:type", String)
], AttributeValue.prototype, "value", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)('Attribute', 'attributeValues', {
        onDelete: 'CASCADE',
    }),
    (0, typeorm_1.JoinColumn)({ name: 'attribute_id' }),
    __metadata("design:type", Object)
], AttributeValue.prototype, "attribute", void 0);
__decorate([
    (0, typeorm_1.OneToMany)('VariantAttribute', 'attributeValue', {
        cascade: true,
    }),
    __metadata("design:type", Array)
], AttributeValue.prototype, "variantAttributes", void 0);
exports.AttributeValue = AttributeValue = __decorate([
    (0, typeorm_1.Entity)('attribute_values'),
    (0, typeorm_1.Index)(['attribute_id', 'value'], { unique: true })
], AttributeValue);
//# sourceMappingURL=attribute-value.entity.js.map